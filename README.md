...
- **Prisma** ORM — **PostgreSQL** (standard transactions; no special topology needed)
...
1. **Database** — create a **PostgreSQL** database (e.g. on the VPS:
   `createdb royal1`) and put its connection string into `DATABASE_URL`
   (`postgresql://USER:PASSWORD@HOST:5432/royal1?schema=public`). Then run
   `npm run db:migrate` (`prisma migrate deploy` — applies the SQL migrations
   in `prisma/migrations`) followed by `npm run db:seed`. For local dev,
   `npm run setup` (`prisma generate && prisma db push && db:seed`) is fine.
