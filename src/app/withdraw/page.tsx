...
import { coins as fmtCoins } from "@/lib/fmt";
    ...
    <div className="space-y-4 py-2">
      ...
      <div className="card p-4">
        ...
        <div className="mt-1 flex items-center gap-2 font-display text-2xl font-extrabold">
          <CoinIcon className="text-game-gold" size={24} /> {fmtCoins(me.balanceFmt)}
        </div>
      ...
      <div className="card space-y-3 p-4">
        ...
        <div className="rounded-xl bg-[#FFF6CC] p-3 text-sm">
      ...
      <div className="card p-4">
        ...
        {rows.length === 0 ? (
          ...
          <div className="space-y-2">
            {rows.map((w) => (
              <div key={w.id} className="rounded-xl bg-black/5 p-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1 text-sm font-semibold">
                    <CoinIcon className="text-game-gold" /> {fmtCoins(w.coinsFmt)}
                    <span className="text-slate-400"> → {w.receiveUsdt.toFixed(2)} USDT</span>
                  </div>
                  <span className={`chip ${STATUS_STYLE[w.status] ?? "bg-black/5 text-slate-400"}`}>
