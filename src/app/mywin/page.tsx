      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="glass p-4">
          ...
          <div className="mt-1 flex items-center gap-1.5 text-2xl font-black text-game-green">
            <CoinIcon className="text-game-gold" /> {coins(summary?.returnedFmt ?? "0")}
          </div>
        </div>
        <div className="glass p-4">
          <div className="text-xs text-slate-400">Total wagered</div>
          <div className="mt-1 flex items-center gap-1.5 text-2xl font-black text-[#111111]">
            <CoinIcon className="text-game-gold" /> {coins(summary?.wageredFmt ?? "0")}
          </div>
        </div>
      </div>
        ...
        {TABS.map((t) => (
          <button
            ...
            className={`shrink-0 rounded-full px-4 py-2 text-sm font-semibold transition ${
              tab === t ? "bg-royal-blue text-white" : "bg-white text-[#2D3987]"
            }`}
          >
      ...
      {busy ? (
        ...
        <div className="space-y-2">
          {bets.map((b) => (
            ...
            >
              <div className="grid h-11 w-11 place-items-center rounded-2xl bg-black/5 text-center">
              ...
              <div className="text-right">
                {b.status === "WON" ? (
                  <span className="flex items-center gap-1 text-sm font-bold text-game-green">
                    +<CoinIcon /> {coins(b.payoutFmt)}
                  </span>
                ) : b.status === "LOST" ? (
                  <span className="flex items-center gap-1 text-sm text-slate-500">
                    −<CoinIcon /> {coins(b.amountFmt)}
                  </span>
                ) : (
