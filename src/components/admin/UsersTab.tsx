...
export function UsersTab() {
  ...
  return (
    <div className="card p-4 md:p-5">
                ...
                <td className="py-3 pr-3">
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      <div className="truncate font-medium text-slate-100 hover:underline">
                        {u.username}
                      </div>
      ...
      {detailId && (
        <UserDetailsDrawer userId={detailId} onClose={() => setDetailId(null)} />
      )}
    </div>
  );
}
...
function Badges({ u }: { u: AdminUser }) {
  return (
    <div className="flex flex-wrap gap-1.5">
      <span
        className={`chip ${
          u.isAdmin ? "bg-game-gold/20 text-game-gold" : "bg-black/5 text-slate-400"
        }`}
      >
...
function Actions({
  ...
  return (
    <div className="flex flex-wrap gap-1.5 md:justify-end">
      ...
      <button
        onClick={() => adjust(u, -1)}
        className="chip bg-black/5 text-slate-300 active:bg-black/[0.07]"
      >
        − coins
      </button>
      <button
