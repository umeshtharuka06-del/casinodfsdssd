    ...
    <div className="mx-auto max-w-md py-10">
      ...
      <div className="card p-6 md:p-8">
        ...
        <p className="mt-1 text-sm text-slate-400">
          {isRegister
            ? "Sign up in seconds and start playing."
        ...
        <form ref={formRef} onSubmit={submit} className="mt-6 space-y-4" noValidate>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <button
                ...
                aria-pressed={showPw}
                className="absolute right-2 top-1/2 grid h-8 w-8 -translate-y-1/2 place-items-center rounded-lg text-slate-400 transition active:bg-black/[0.07]"
              >
          ...
          {error && (
            <div
              role="alert"
              className="rounded-lg border border-game-red/40 bg-game-red/10 px-3 py-2 text-sm text-[#D81E2C]"
            >
              {error}
            </div>
          )}
        ...
        <div className="mt-5 text-center text-sm text-slate-400">
          {isRegister ? (
            <>
              ...
              <Link href="/login" className="font-semibold text-royal-blue-bright">
                Log in
              </Link>
            </>
          ) : (
            <>
              New to Mega 99?{" "}
              <Link href="/register" className="font-semibold text-royal-blue-bright">
                Create one
              </Link>
            </>
          )}
