    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          ...
          <Stat label="Today's bets" value={s.live.todaysBets.toLocaleString()} accent="text-[#111111]" />
      ...
      <div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          ...
          <Stat label="Pending deposits" value={s.queues.pendingDeposits.toLocaleString()} accent={s.queues.pendingDeposits > 0 ? "text-game-gold" : "text-[#111111]"} />
          <Stat label="Pending withdrawals" value={s.queues.pendingWithdrawals.toLocaleString()} accent={s.queues.pendingWithdrawals > 0 ? "text-game-red-bright" : "text-[#111111]"} />
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Stat label="Total bets" value={s.totals.bets.toLocaleString()} accent="text-[#111111]" />
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Stat label="Wagered (24h)" value={s.last24h.wageredFmt} coin accent="text-[#111111]" />
        ...
        <div className="card p-5 lg:col-span-2">
          ...
          <div className="mt-3 grid grid-cols-2 gap-x-6 gap-y-2 text-sm sm:grid-cols-3">
            {s.roundsByGame.map((g) => (
              <div key={g.game} className="flex justify-between border-b border-black/5 pb-1">
                <span className="text-slate-300">{g.game}</span>
                <span className="font-semibold text-[#111111]">{g.rounds.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Live bet feed */}
      <div className="card p-5">
        <h3 className="mb-3 text-sm font-bold text-[#111111]">Live bet feed</h3>
              ...
              {s.recentBets.map((b) => (
                <tr key={b.id}>
                  ...
                  <td className="whitespace-nowrap pr-3 text-slate-200">
                    <CoinIcon className="text-game-gold" /> {coins(b.amountFmt)}
                  </td>
                  <td className="pr-3">
                    <StatusBadge status={b.status} />
                  </td>
                  <td className="whitespace-nowrap pr-3 text-slate-200">
                    <CoinIcon className="text-game-gold" /> {coins(b.payoutFmt)}
                  </td>
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, string> = {
    ...
    LOST: "bg-black/5 text-slate-400",
    PENDING: "bg-royal-blue/15 text-royal-blue-bright",
  };
  return <span className={`chip ${map[status] ?? "bg-black/5 text-slate-300"}`}>{status}</span>;
}
