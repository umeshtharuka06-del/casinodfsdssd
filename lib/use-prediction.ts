...
export function usePredictionMode(
  ...
  // PARITY no longer flows through this hook (it has a dedicated flow in
  // use-parity.ts); `enabled` lets the page keep both hooks mounted and idle
  // the inactive one, since hooks cannot be mounted conditionally.
  enabled = true
) {
  ...
  // Reset when the mode (tab) changes or the hook is idled so we don't
  // false-fire onNewPeriod or keep serving another mode's stale data.
  useEffect(() => {
