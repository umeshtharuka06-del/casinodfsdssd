...
async function aggregateRoundBets(
  ...
  const [withFee, legacy, players] = await Promise.all([
    ...
    // groupBy caps the work in PostgreSQL and returns at most 2 rows.
    prisma.bet.groupBy({
      by: ["userId"],
      where: { roundId },
      orderBy: { userId: "asc" },
...
export async function settleDuePredictionRounds(mode: PredictionMode) {
  ...
  // Prisma build might reject), so it cannot silently break settlement.
  let due;
  try {
  ...
  for (const round of due) {
    ...
    try {
      ...
      // STAGE: BETTING — round picked up for settlement (with DB identity).
      lc.trace("BETTING", {
        dbId: round.id, mode, period, state: round.state, bets: totalBets,
      ...
      lc.trace("RESULT_GENERATED", { dbId: round.id, mode, period, digit: res.digit, colors: res.colors, decisionMode: res.mode, bets: totalBets });
      ...
      if (r.count === 0) {
        ...
        // STAGE: SETTLED — with the actual DB write count (proves the write).
        lc.stage(round.id, "SETTLED");
        lc.trace("SETTLED", { dbId: round.id, mode, period, digit: res.digit, dbWriteCount: r.count });
      ...
      lc.trace("PAYOUT_COMPLETED", { dbId: round.id, mode, period });
    } catch (e) {
 ...
 * live DB state of the round (state / settledAt / result — proves whether the DB
