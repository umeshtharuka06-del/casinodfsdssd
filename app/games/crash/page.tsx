...
interface Round {
  ...
  startAt: string;
  settleAt: string;
  serverNow?: string; // engine clock at fetch time — used to align the animation
}
interface MyBet {
  id: string;
  amountFmt: string;
...
export default function CrashGamePage() {
  ...
  // Offset (ms) to convert the local clock into the engine's clock, so the live
  // multiplier is aligned to the SERVER and can never drift from the backend.
  ...
  const load = useCallback(async () => {
    ...
    if (res.ok && res.data) {
      // Align our clock to the engine's using the server timestamp it returns,
      // so the animation is driven by SERVER time (no client-clock drift).
      if (res.data.round?.serverNow) {
        serverOffsetRef.current = Date.parse(res.data.round.serverNow) - Date.now();
      }
  ...
  // Live multiplier — driven ENTIRELY by the backend round window, never by an
  // independent client animation. The engine sets settleAt = startAt +
  // durationMsForCrash(crashX), so multiplierAt(settleAt - startAt) === the
  // engine's crashX. We therefore:
  //   • run on SERVER time (serverOffsetRef) so the client clock can't drift,
  //   • CLAMP the live value so it can never exceed the engine crash point,
  //   • STOP exactly at settleAt and freeze on that exact crash multiplier —
  //     the same value the engine records and Recent Crashes shows.
  useEffect(() => {
    if (!round) return;
    const srv = now + serverOffsetRef.current;
    const startT = new Date(round.startAt).getTime();
    const settleT = new Date(round.settleAt).getTime();
    const crashMult = multiplierAt(settleT - startT); // == engine crashX

    if (srv < startT) {
      setDisplay(1); // betting / countdown
      return;
    }
    if (srv >= settleT) {
      setDisplay(crashMult); // crashed — freeze on the engine's exact multiplier
      return;
    }
    // Running: clamp so the animation can never overshoot the backend result.
    const m = Math.min(multiplierAt(srv - startT), crashMult);
    setDisplay(m);
    if (
  ...
  // All phase gates run on the engine-aligned clock and the round's start/settle
  // timestamps, so the countdown, live multiplier and "Busted at" banner flip at
  // exactly the moments the backend does — no independent client timeline.
  const srvNow = now + serverOffsetRef.current;
  const startMs = round ? new Date(round.startAt).getTime() : 0;
  const settleMs = round ? new Date(round.settleAt).getTime() : 0;
  const betCountdown = round ? Math.max(0, Math.ceil((startMs - srvNow) / 1000)) : 0;
  const phase = !round ? "…" : srvNow < startMs ? "BETTING" : srvNow < settleMs ? "RUNNING" : "SETTLED";
  const crashed = round ? srvNow >= settleMs && srvNow >= startMs : false;
  const won = myBet?.status === "CASHED";
  const lost = myBet?.status === "LOST";
  const running = phase === "RUNNING" && myBet?.status === "PENDING";
      ...
      <div className="space-y-5">
        ...
        >
          <div
            ...
            style={{
              background: crashed
                ? "radial-gradient(60% 60% at 50% 50%, rgba(216,30,44,0.12), transparent)"
                : "radial-gradient(60% 60% at 50% 40%, rgba(76,108,6,0.15), transparent)",
            }}
          />
          <div className="relative z-10 text-center">
            ...
            <div
              className={`mt-1 font-display font-black tabular-nums leading-none transition-all ${
                crashed ? "text-game-red" : "text-[#111111]"
              }`}
            ...
            <>
              ...
              <div className="mt-2 flex gap-2">
                {CHIPS.map((v) => (
                  <button
                    ...
                    onClick={() => setAmount(v)}
                    disabled={abActive}
                    className="flex-1 rounded-lg border border-black/10 bg-black/5 px-2 py-1 text-xs disabled:opacity-50"
                  >
                    {v}
                  </button>
                ))}
              ...
              <div className="mt-4 flex flex-wrap items-center gap-3">
                {running ? (
                  ...
                  <button
                    onClick={manualBet}
                    disabled={phase !== "BETTING" || !!myBet || abActive}
              ...
              <div className="mt-4 space-y-4">
                ...
                {abActive && (
                  <div className="flex flex-wrap gap-2 text-xs">
                    <span className="chip bg-black/5 text-slate-300">Rounds {stats.rounds}</span>
                    ...
                    <span className="chip bg-black/5 text-slate-300">Next stake {stats.stake}</span>
                  </div>
                )}
    ...
    <button
      ...
      className={`flex items-center justify-between rounded-xl border px-3 py-2.5 text-xs font-semibold transition disabled:opacity-60 ${
        on
          ...
          : "border-black/10 bg-[#FFF6CC] text-slate-300"
      }`}
    >
      {label}
      <span>{on ? "ON" : "OFF"}</span>
